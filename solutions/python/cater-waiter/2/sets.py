...
def clean_ingredients(dish_name: str, dish_ingredients):
...
def categorize_dish(dish_name, dish_ingredients):
    ...
    for item in ((VEGAN, "VEGAN"), (VEGETARIAN, "VEGETARIAN"),
                 (PALEO, "PALEO"), (KETO, "KETO"), (OMNIVORE, "OMNIVORE")):
        if dish_ingredients <= item[0]:
            return ": ".join([dish_name, item[1]])
...
def singleton_ingredients(dishes: list, intersection: set):
    ...
    singleton = set()
    for dish in dishes:
        others = set().union(*(d for d in dishes if d is not dish))
        singleton.update(dish - others)
